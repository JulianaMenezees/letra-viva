// LevelsCacaPalavras.js (ajustado para 4 níveis e navegando para 'Dominio')
...
export default function LevelsCacaPalavras({ navigation, route }) {
    ...
    // por padrão navegar para 'Dominio' (nome da screen). Se você usa outro nome, passe via route.params.gameRouteName
    const GAME_ROUTE = route?.params?.gameRouteName || "DominoScreen";
    const PROGRESS_KEY = route?.params?.progressKey || "dominio_progress_global";
    ...
    function renderItem({ item: level }) {
        ...
        return (
            <TouchableOpacity
                ...
                onPress={() => {
                    if (!unlocked) return;
                    // navegamos para Dominio passando somente dados primitivos (level & progressKey)
                    navigation.navigate('DominoScreen', {
                        level,
            ...
            <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
                <ImageBackground source={{ uri: TOP_IMAGE }} style={styles.topImage} imageStyle={{ opacity: 0.12 }}>
                    <View style={styles.topBar}>
                        <Image source={require("../assets/images/jogos/cacaPalavras/domino.png")} style={styles.titleImage} resizeMode="contain" />
                    </View>
                </ImageBackground>
